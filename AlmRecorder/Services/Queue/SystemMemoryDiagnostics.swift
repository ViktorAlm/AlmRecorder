...
enum SystemMemoryDiagnostics {
    ...
    /// Physical footprint for one child process. Nightly model workers use the same metric as the
    /// queue diagnostics so the displayed current/peak RAM is directly comparable.
    static func physicalFootprint(pid: pid_t) -> UInt64? {
