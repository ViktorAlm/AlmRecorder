...
struct QueueView: View {
    ...
    var body: some View {
        ...
        .onChange(of: queueManager.completedJobs.count) {
...
private struct MemoryUsageBreakdownView: View {
    ...
    private func memoryReadiness(_ system: SystemMemorySnapshot) -> some View {
        ...
        let expectedPaging = requirements.expectedPagingBytes(for: system)
        ...
        return VStack(alignment: .leading, spacing: 7) {
            ...
            HStack {
                ...
                Spacer()
                Text("\(formatBytes(requirements.requiredHeadroomBytes)) minimum")
            }
            .font(.caption2)
            .foregroundColor(.secondary)
            .monospacedDigit()
            ...
            if headroomShortfall > 0 {
                requirementWarning(
                    "Need \(formatBytes(headroomShortfall)) more headroom before controlled paging is safe."
                )
            }
            ...
            if headroomShortfall == 0,
               ...
               physicalShortfall == 0 {
                if expectedPaging > 0 {
                    Label(
                        "Elastic launch ready: macOS may compress or swap up to "
                            + "\(formatBytes(expectedPaging)) as the model grows "
                            + "(\(formatBytes(requirements.idealHeadroomBytes)) ideal headroom).",
