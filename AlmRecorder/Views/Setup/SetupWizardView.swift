...
struct SetupWizardView: View {
    ...
    @ObservedObject private var realtimeDictationModels = VibeASRBitNetModelManager.shared
    ...
    private var realtimeDictationModelInstalled: Bool {
        realtimeDictationModels.isModelDownloaded
    }
    private var allRecommendedModelsInstalled: Bool {
        transcriptionModelInstalled
            && realtimeDictationModelInstalled
            && gemmaModelInstalled
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 0) {
                ...
                modelRow(
                    ...
                    name: "VibeVoice ASR BitNet · local hold-to-talk",
                    size: bytesString(realtimeDictationDownloadBytes),
                    done: realtimeDictationModelInstalled
                )
                if realtimeDictationModels.isDownloading {
                    ProgressView(value: realtimeDictationModels.downloadProgress)
                        .padding(.horizontal, 12)
                    Text(realtimeDictationModels.status)
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .padding(.horizontal, 12)
                        .padding(.bottom, 10)
    ...
    private func startDownloads() {
        ...
        if !realtimeDictationModels.isModelDownloaded {
            Task {
                do {
                    try await realtimeDictationModels.downloadModel()
                    await MainActor.run {
                        RealtimeDictationController.shared.warm()
                    }
                } catch {
                    await MainActor.run {
                        downloadError = "Realtime dictation model download failed: \(error.localizedDescription)"
                    }
                }
            }
        }
    ...
    private var realtimeDictationDownloadBytes: Int64 {
        RealtimeDictationConfiguration.manifest.files.reduce(0) {
            $0 + $1.minimumBytes
        }
    }
    private var remainingDownloadGB: Double {
        (transcriptionModelInstalled ? 0 : Double(plan.vibeVoiceQuantization.estimatedDownloadBytes) / 1_000_000_000.0)
            + (realtimeDictationModelInstalled ? 0 : Double(realtimeDictationDownloadBytes) / 1_000_000_000.0)
